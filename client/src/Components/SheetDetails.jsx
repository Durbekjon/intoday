...
export default function SheetDetails({ sheet, getSheetById }) {
    ...
    const [rows, setRows] = useState(sheet?.tasks);
    ...
    const handleAddRow = async () => {
        try {
            const { data } = await axiosInstance.post(`/task`, { name: '', editor: '', status: '', uploadStatus: '', deadline: '', price: '', paid: false, sheet: sheet._id });
            setRows([...rows, { name: '', editor: '', status: '', uploadStatus: '', deadline: '', price: '', paid: false }])
            console.log(data);
        } catch {
    ...
    return (
        <>
            <div className='p-[15px_0px_0px_15px] mainn '>
                <div className='flex gap-[10px] mb-[40px] justify-between  items-center'>
                ...
                <Tables rows={rows} setRows={setRows} sheet={sheet.tasks} />
            </div>
        </>
    );
}
